...
The dedicated Apple encryption key was provisioned separately for production and preview/development on September 5, 2026. Preserve it: rotating or losing it without re-encrypting existing grants and deletion work prevents those records from being processed. Signing/provider configuration was completed September 6 UTC; consult [the current provider setup](../implementation/morpheus-apple-provider-setup.md) and [grant/deletion configuration](../implementation/morpheus-cloud-apple-deletion.md). No Apple secret belongs in public configuration.
...
Production is deployment `dpl_DdNL3SRvafnu6ye68MDx7i4HyXeS`, commit `3401eff0d4b9be50e669603d8f70f5ffffaf6a7f`, rebuilt September 6 UTC with the Apple signing environment. It was built for the production target with `autoAssignCustomDomains:false`, checked before promotion, then explicitly promoted and verified at `www.soapbubble.xyz`. The preceding cloud release `dpl_HRwZcXNyfyjbXVxgyVZuTcrvXfxs` lacks those Apple signing settings; account for that before rolling back Apple functionality. Rolling back must not drop the cloud database.

Preview `dpl_GkWkqo6cYdgbzf3Xvnppd755e14P` uses the same code with development resources and protected Apple signing settings. Its stable webhook alias is `morpheus-web-www-git-codex-morpheus-cloud-flicks-projects.vercel.app`. Verify aliases explicitly after cross-target deployments: a redeploy can inherit its source branch alias.
...
Apple Developer access was restored, and the native primary App ID capability was verified already saved. Both Clerk environments now use the approved Morpheus signing key and Services ID. The production Apple button reaches Apple's authorization page. Real native/web identity parity (including Hide My Email), token exchange/revocation, signed physical-device verification and final App Store privacy metadata remain unverified. These requirements must be completed before the native Apple-authenticated release. See [provider evidence and credential handling](../implementation/morpheus-apple-provider-setup.md).
