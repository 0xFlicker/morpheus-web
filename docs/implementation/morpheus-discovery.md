...
The descriptive comparison uses each **other player's best currently saved completed playthrough**, interpreted with the current authored catalog, with at least **20 distinct other server-owned player identities**. It excludes the current player's records, imports, incomplete runs, and expired guests. Multiple slots or linked guest/account records cannot multiply that player's weight. Anonymous identities can still represent multiple installations of the same human.
